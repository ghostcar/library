...
7. Авторский каталог сохраняется целиком как observations для обнаружения и
   reconciliation, но `new_release` создаётся только для записи, детерминированно
   связанной с канонической серией с enabled watch-backed metadata source. Книги
   остальных серий автора и записи без надёжной связи с серией не уведомляют.
