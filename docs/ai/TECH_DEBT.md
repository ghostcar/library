...
| 21 | ~~SSR did not use the persisted refresh token~~ РЕШЕНО И РАЗВЁРНУТО: expired access redirects through allowlisted `/auth/session`, reissues access without cross-tab refresh rotation and returns to the requested portal page. | — | — | — | — |
