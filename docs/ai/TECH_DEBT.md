...
| 19 | Source polling has no manual `Проверить сейчас`, next-run/result indicator, or persistent check history; exact outcomes are visible only in worker logs | owner cannot confidently tell whether a newly connected page has refreshed | high | agent | next source-monitoring UX slice |
