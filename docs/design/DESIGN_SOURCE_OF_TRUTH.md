...
Версия: 0.2 (reconciled 2026-08-28; детальный разбор экранов — по мере реализации UI).
...
|---------|---------|----------|
| Цветовые tokens | Обе темы из ghostcar-design/tokens.md, локальные CSS-переменные и компоненты; Tailwind осознанно не добавлялся | ghostcar-design |
...
1. Базовая тема: реализована Astral по умолчанию + доступный переключатель Solar; решение закрыто `OPEN_QUESTIONS#6`.
...
3. Насколько плотно воспроизводить орбитальные визуализации серий (desktop_4) vs простой таймлайн — решение: компактные status/evidence chips, без декоративной псевдовизуализации (TaskContext `visual-series-state-2026-08-28`).
4. Итоговый набор пунктов sidebar: реализован desktop с полным набором разделов
